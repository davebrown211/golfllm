...
async function getPowerRankings() {
  const client = await pool.connect()
  try {
    // Read the shared power rankings query
    const fs = require('fs')
    const path = require('path')
    const queryPath = path.join(process.cwd(), '..', '..', 'shared', 'power-rankings-query.sql')
...
export default async function Home() {
  const [curatedVideos, instructionalVideos, professionalVideos, discoveryVideos, stats, videosWithAudio, videoOfTheDay, powerRankings] = await Promise.all([
    getCuratedVideos(),
  ...
  return (
    <HomePage 
      ...
      initialPowerRankings={powerRankings}
    />
  )
}